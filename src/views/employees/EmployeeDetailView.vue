...
const top5Strengths = computed(() => {
  ...
  return getTop5Strengths(currentEmployee.value.strengths.strengths_order)
})
      ...
      <template v-else>
          ...
          <div class="flex flex-col md:flex-row md:items-center gap-6">
            ...
            <div class="flex-1">
              ...
              <p class="text-gray-600 dark:text-gray-400 mt-1">
                {{ currentEmployee.job_title || JOB_TYPE_LABELS[currentEmployee.job_type || 'other'] }}
              </p>
        ...
        <div class="card">
          ...
          <div v-if="activeTab === 'profile'" class="space-y-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div>
                ...
                <p class="mt-1 text-gray-900 dark:text-white">{{ currentEmployee.name_kana || '-' }}</p>
              </div>
              <div>
              ...
              <div>
                ...
                <p class="mt-1 text-gray-900 dark:text-white">
                  {{ JOB_TYPE_LABELS[currentEmployee.job_type || 'other'] }}
                </p>
              </div>
              <div>
                <label class="text-sm text-gray-500 dark:text-gray-400">役職</label>
                <p class="mt-1 text-gray-900 dark:text-white">{{ currentEmployee.job_title || '-' }}</p>
              </div>
              <div>
                <label class="text-sm text-gray-500 dark:text-gray-400">入社日</label>
                <p class="mt-1 text-gray-900 dark:text-white">{{ formatDate(currentEmployee.hire_date) }}</p>
              </div>
              <div>
            ...
            <div v-if="currentEmployee.careers && currentEmployee.careers.length > 0" class="mt-8">
              ...
              <div class="space-y-4">
                ...
                >
                  ...
                  <p class="text-sm text-gray-600 dark:text-gray-400">{{ career.company_name }}</p>
          ...
          <div v-else-if="activeTab === 'howto'" class="space-y-6">
            ...
            <div v-if="currentEmployee.ai_profile" class="space-y-6">
              <div class="p-4 bg-primary-50 dark:bg-primary-900/10 rounded-lg">
                ...
                <p class="text-gray-700 dark:text-gray-300">{{ currentEmployee.ai_profile.profile_summary }}</p>
              </div>

              <div>
                <h3 class="font-medium text-gray-900 dark:text-white mb-2">ワークスタイル分析</h3>
                <p class="text-gray-600 dark:text-gray-400">{{ currentEmployee.ai_profile.work_style_analysis }}</p>
              </div>

              <div>
                <h3 class="font-medium text-gray-900 dark:text-white mb-2">協働のコツ</h3>
                <p class="text-gray-600 dark:text-gray-400">{{ currentEmployee.ai_profile.collaboration_tips }}</p>
              </div>

              <div>
                <h3 class="font-medium text-gray-900 dark:text-white mb-2">成長提案</h3>
                <p class="text-gray-600 dark:text-gray-400">{{ currentEmployee.ai_profile.development_suggestions }}</p>
              </div>

              <p class="text-xs text-gray-400 dark:text-gray-500">
                生成日: {{ formatDate(currentEmployee.ai_profile.generated_at) }}
              </p>
            </div>
            ...
            <div v-if="currentEmployee.strengths" class="space-y-6">
              ...
              <div>
                ...
                <div class="flex flex-wrap gap-2">
                  <span
                    v-for="(strengthId, index) in currentEmployee.strengths.strengths_order.slice(0, 10)"
                    :key="strengthId"
          ...
          <div v-else-if="activeTab === 'spi'" class="space-y-6">
            ...
            <div v-if="currentEmployee.spi_results" class="space-y-8">
              <!-- Personality traits -->
              <div>
                <h3 class="font-medium text-gray-700 dark:text-gray-300 mb-4">性格特性</h3>
                <div class="space-y-4">
                  <div
                    v-for="(value, key) in currentEmployee.spi_results.personality_traits"
                    :key="key"
                  >
              ...
              <div>
                ...
                <div class="space-y-4">
                  <div
                    v-for="(value, key) in currentEmployee.spi_results.work_style"
                    :key="key"
                  >
                    <div class="flex justify-between text-sm mb-1">
              ...
              <div>
                ...
                <div class="grid grid-cols-3 gap-4">
                  <div
                    v-for="(value, key) in currentEmployee.spi_results.aptitude_scores"
                    :key="key"
              ...
              <p class="text-xs text-gray-400 dark:text-gray-500">
                テスト日: {{ formatDate(currentEmployee.spi_results.test_date) }}
              </p>
            </div>
            <div v-else class="text-center py-8">
                ...
                >
                  <div class="flex items-center justify-between mb-4">
                    ...
                    <span
                      ...
                      :class="gradeColors[evaluation.overall_grade]"
                    >
                      {{ evaluation.overall_grade }}
                    </span>
                  </div>
                  <div class="space-y-3 text-sm">
                    <div v-if="evaluation.strengths_comment">
                      <span class="text-gray-500 dark:text-gray-400">強み:</span>
                      <p class="text-gray-700 dark:text-gray-300 mt-1">{{ evaluation.strengths_comment }}</p>
                    </div>
                    <div v-if="evaluation.improvements_comment">
                      <span class="text-gray-500 dark:text-gray-400">改善点:</span>
                      <p class="text-gray-700 dark:text-gray-300 mt-1">{{ evaluation.improvements_comment }}</p>
                    </div>
